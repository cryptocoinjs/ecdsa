...
function recoverPubKey(e, signature, i) {
  ...
  var n = curve.n
  var G = curve.g
  var pcurve = curve.curve
  var p = pcurve.q
  var a = pcurve.a.toBigInteger()
  var b = pcurve.b.toBigInteger()

  // We precalculate (p + 1) / 4 where p is the field order
  if (!pcurve.P_OVER_FOUR) {
    pcurve.P_OVER_FOUR = p.add(BigInteger.ONE).shiftRight(2)
  }
  ...
  var beta = alpha.modPow(pcurve.P_OVER_FOUR, p)
  ...
  var R = new ECPointFp(pcurve, pcurve.fromBigInteger(x), pcurve.fromBigInteger(y))
  R.validate()
...
function verify(hash, signature, pubkey) {
  ...
  var Q;
   if (Buffer.isBuffer(pubkey)) {
    Q = ECPointFp.decodeFrom(curve.curve, pubkey);
  } else {
